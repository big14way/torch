...
| Contract | Address |
| --- | --- |
| TorchVaultV2 (live, Aug 6) | [`0x8d9A6a11BcC64CC36e54b22ACa68865d759fa6Bd`](https://coston2-explorer.flare.network/address/0x8d9A6a11BcC64CC36e54b22ACa68865d759fa6Bd) |
| FtsoV2Reader (registry-resolving, live) | [`0xF8be9ca7bCb07B0e87BDcEAB28841fF6A16E7D01`](https://coston2-explorer.flare.network/address/0xF8be9ca7bCb07B0e87BDcEAB28841fF6A16E7D01) |
...
| TorchFdcConsumer (bound to v2) | [`0xf8A523571D3b6b41C5203c4D7EE32648CAfEcbff`](https://coston2-explorer.flare.network/address/0xf8A523571D3b6b41C5203c4D7EE32648CAfEcbff) |
...
- The flow is scripted and permissionless: `npm run fdc:attest -w contracts` (latest fill) or `POSITION_ID=<id> npm run fdc:attest -w contracts` (position-bound) — prepare the request at the FDC verifier, submit to `FdcHub`, wait the voting round (~2-3 min), pull the Merkle proof from the DA layer, then the consumer verifies it through `ContractRegistry.getFdcVerification()`. One honest note: the July receipts above live on the retired v1 consumer, which is replay-guarded, so re-running those exact targets there reverts with "already attested". The live consumer is bound to v2 and fresh — `npm run fdc:attest` against it attests the account's latest real fill.
