...
| FDC fill attestation | **Proven on-chain** — incl. the first live position validator-bound to its fill (Aug 6) | tx receipts below |
...
3. Positive PnL is paid from an explicit on-chain insurance fund, and payouts are capped at its live balance (`PayoutCapped`) — stated here rather than discovered at withdrawal. On testnet that balance is small (~10 FXRP at v2 launch, fed by faucet drips), so treat it as the current ceiling on any single win. Negative PnL accrues to the fund. Nothing is hidden in an off-chain promise.
...
| TorchFdcConsumer (bound to v2) | [`0xf8A523571D3b6b41C5203c4D7EE32648CAfEcbff`](https://coston2-explorer.flare.network/address/0xf8A523571D3b6b41C5203c4D7EE32648CAfEcbff#code) (source-verified) |
...
- The flow is scripted and permissionless: `npm run fdc:attest -w contracts` (latest fill) or `POSITION_ID=<id> npm run fdc:attest -w contracts` (position-bound) — prepare the request at the FDC verifier, submit to `FdcHub`, wait the voting round (~2-3 min), pull the Merkle proof from the DA layer, then the consumer verifies it through `ContractRegistry.getFdcVerification()`. The live v2-bound consumer already holds both receipt shapes from Aug 6: the canary's close fill attested standalone (tx [`0x95764f39…c243`](https://coston2-explorer.flare.network/tx/0x95764f39d8768a7f037ec794eb3a1776f66b33969c4b8f65affc339ebf03c243)) and position #1 bound to its open fill (tx above). One honest note: the consumer is replay-guarded, so re-running an already-attested target reverts — a from-scratch run needs a fresh exchange fill, which any venue-routed trade produces.
